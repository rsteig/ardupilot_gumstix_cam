...
AP_Camera::transistor_pic()
{
    ...
    hal.gpio->write(AP_CAMERA_TRANSISTOR_PIN,1);
    	...
    	float alt = _current_loc->alt;
    	float lat = _current_loc->lat;
    	float lng = _current_loc->lng;
		hal.uartC->println("pic");
		hal.uartC->println(alt);
		hal.uartC->println(lat);
		hal.uartC->println(lng);
	#endif
